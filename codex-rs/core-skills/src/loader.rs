...
fn canonicalize_for_skill_identity(path: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
    path.canonicalize().ok()
}

fn skill_identity_path(path: &AbsolutePathBuf) -> AbsolutePathBuf {
    canonicalize_for_skill_identity(path).unwrap_or_else(|| path.clone())
}
...
async fn discover_skills_under_root(
    ...
    let root = skill_identity_path(root);
        ...
        for entry in entries {
            ...
            if metadata.is_symlink {
                ...
                match fs.read_directory(&path, /*sandbox*/ None).await {
                    Ok(_) => {
                        let Some(resolved_dir) = canonicalize_for_skill_identity(&path) else {
                            tracing::debug!(
                                path = %path.display(),
                                "skipping symlink dir without canonical local path"
                            );
                            continue;
                        };
                        enqueue_dir(
            ...
            if metadata.is_directory {
                let resolved_dir = skill_identity_path(&path);
                enqueue_dir(
                    &mut queue,
                    &mut visited_dirs,
                    &mut truncated_by_dir_limit,
...
async fn parse_skill_file(
    ...
    let resolved_path = skill_identity_path(path);
