...
fn assert_call_create_multipart(
    ...
    let session_payload = body
        .strip_prefix(&expected_prefix)
        .context("multipart body should contain the expected sdp part")?
        .strip_suffix("\r\n--codex-realtime-call-boundary--\r\n")
        .context("multipart body should end with the final boundary")?;
    let actual_session: Value =
        serde_json::from_str(session_payload).context("session part should be valid json")?;
