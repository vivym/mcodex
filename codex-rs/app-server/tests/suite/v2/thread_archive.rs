...
#[tokio::test]
async fn thread_archive_succeeds_after_thread_start_materializes_rollout() -> Result<()> {
    ...
    assert!(
        rollout_path.exists(),
        "thread/start should materialize rollout at {}",
        rollout_path.display()
    );

    let discovered_path = find_thread_path_by_id_str(codex_home.path(), &thread.id)
        .await?
        .expect("expected rollout path for thread id to exist after thread/start");
