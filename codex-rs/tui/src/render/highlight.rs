...
//! | `MCODEX_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static MCODEX_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
pub(crate) fn set_theme_override(
    ...
    let codex_home_set_ok = MCODEX_HOME.set(codex_home.clone()).is_ok();
...
pub(crate) fn validate_theme_name(name: Option<&str>, codex_home: Option<&Path>) -> Option<String> {
    ...
    let custom_theme_path_display = codex_home
        ...
        .unwrap_or_else(|| format!("$MCODEX_HOME/themes/{name}.tmTheme"));
...
fn resolve_theme_with_override(name: Option<&str>, codex_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {MCODEX_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = codex_home
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = MCODEX_HOME.get()
...
/// `.tmTheme` file under `{MCODEX_HOME}/themes/`.
