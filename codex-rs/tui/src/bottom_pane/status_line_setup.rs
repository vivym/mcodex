...
mod tests {
    ...
    fn preview_line(
        ...
        preview_data.line_for_items(items.iter().cloned().map(StatusLineItem::preview_item))
    }
    ...
    fn preview_uses_runtime_values() {
        let preview_data = StatusSurfacePreviewData::from_iter([
            (StatusSurfacePreviewItem::Model, "gpt-5".to_string()),
            (StatusSurfacePreviewItem::CurrentDir, "/repo".to_string()),
        ]);
        assert_eq!(
            preview_line(
                &[StatusLineItem::ModelName, StatusLineItem::CurrentDir],
                preview_data,
            ),
    ...
    fn preview_omits_items_without_runtime_values() {
        let preview_data =
            StatusSurfacePreviewData::from_iter([(StatusSurfacePreviewItem::Model, "gpt-5")]);
        assert_eq!(
            preview_line(
                &[StatusLineItem::ModelName, StatusLineItem::GitBranch],
                preview_data,
            ),
    ...
    fn preview_includes_thread_title() {
        ...
        assert_eq!(
            preview_line(
                &[StatusLineItem::ModelName, StatusLineItem::ThreadTitle],
                preview_data,
            ),
