...
impl CodexAuth {
    ...
    pub fn agent_identity_authorization_header(&self) -> std::io::Result<Option<String>> {
        match self {
            Self::AgentIdentity(auth) => auth.authorization_header().map(Some),
            Self::ApiKey(_) | Self::Chatgpt(_) | Self::ChatgptAuthTokens(_) => Ok(None),
        }
    }
...
impl AuthManager {
    ...
    pub async fn chatgpt_authorization_header_for_auth(&self, auth: &CodexAuth) -> Option<String> {
        ...
        CodexAuth::chatgpt_bearer_authorization_header_for_auth(auth)
    }
    ...
    async fn refresh_external_auth(
        ...
        let previous_account_id = self
            ...
            .as_ref()
            .and_then(CodexAuth::get_account_id);
        let previous_agent_identity = self
            .auth_cached()
            .and_then(|auth| auth.get_current_auth_json())
            .and_then(|auth_dot_json| auth_dot_json.agent_identity);
        ...
        auth_dot_json.agent_identity = previous_agent_identity;
        save_auth(
            &self.codex_home,
            &auth_dot_json,
            AuthCredentialsStoreMode::Ephemeral,
