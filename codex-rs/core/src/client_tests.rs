...
async fn drain_stream_to_completion(stream: &mut ResponseStream) -> anyhow::Result<()> {
...
async fn direct_request_setup_uses_leased_auth_snapshot_without_refresh() {
    ...
    assert_eq!(client_setup.account_id.as_deref(), Some("account_id"));
...
async fn responses_http_setup_acquires_admission_for_pooled_runtime_host() {
    ...
    assert_eq!(setup.setup.account_id.as_deref(), Some("account_id"));
    assert_eq!(
