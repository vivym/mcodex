...
fn resume_command_prefers_name_over_id() {
    ...
    assert_eq!(command, Some("mcodex resume my-thread".to_string()));
}
...
fn resume_command_quotes_thread_name_when_needed() {
    ...
    assert_eq!(
        command,
        Some("mcodex resume -- -starts-with-dash".to_string())
    );

    let command = resume_command(Some("two words"), /*thread_id*/ None);
    assert_eq!(command, Some("mcodex resume 'two words'".to_string()));

    let command = resume_command(Some("quote'case"), /*thread_id*/ None);
    assert_eq!(command, Some("mcodex resume \"quote'case\"".to_string()));
}
