...
-> anyhow::Result<()> {
    ...
    assert_eq!(child_admitted.setup.account_id.as_deref(), Some(account_id));
    let mut child_auth_headers = http::HeaderMap::new();
    child_admitted
        .setup
        .api_auth
        .add_auth_headers(&mut child_auth_headers);
    let expected_authorization = format!("Bearer {}", fake_access_token(account_id));
    assert_eq!(
        child_auth_headers
