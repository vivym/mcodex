...
async fn execute_mcp_tool_call(
    ...
    let result = tokio::select! {
        _ = cancellation_token.cancelled() => {
            return Err(format!("cancelled MCP tool call `{server}/{tool_name}`"));
        }
        result = sess.call_tool(server, tool_name, rewritten_arguments, request_meta) => {
            result.map_err(|e| format!("tool call error: {e:?}"))?
        }
    };
