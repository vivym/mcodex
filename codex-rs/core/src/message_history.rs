...
//!
//! The history is stored at `~/.mcodex/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside the active `mcodex` home.
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.mcodex/history.jsonl` and ensure the parent directory exists.
