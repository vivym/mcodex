...
async fn wait_for_mcp_tool_available(
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let fixture = test_codex()
        ...
        .await?;
    let session_model = fixture.session_configured.model.clone();
    wait_for_mcp_tool_available(&fixture.codex, &tool_name).await?;
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    let server_name = "rmcp";
    let namespace = format!("mcp__{server_name}__");
    let tool_name = format!("{namespace}sync");
...
async fn stdio_image_responses_are_sanitized_for_text_only_model() -> anyhow::Result<()> {
    ...
    let server_name = "rmcp";
    let namespace = format!("mcp__{server_name}__");
    let tool_name = format!("{namespace}image");
...
/// This test writes to a fallback credentials file in the product home.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire home env through the code.
#[test]
#[serial(codex_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    let _mcodex_home_guard = EnvVarGuard::set("MCODEX_HOME", temp_home.path().as_os_str());
